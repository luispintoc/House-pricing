...
# Optimize model
classifier = 'lasso'
cv = RepeatedKFold(n_splits=6, random_state=100)
train = False
...
if train:
	...
	best = {'alpha': 0.0014198486022931078, 'max_iter': 50000}

trained_clf = Lasso().set_params(**best).fit(x_train, y_train)
print(trained_clf.predict(x_test))
y_test = np.exp(trained_clf.predict(x_test))


# ss = pd.DataFrame({'Id': id_test, 'SalePrice': y_test})
# ss.to_csv('submit_lasso.csv', index=False)